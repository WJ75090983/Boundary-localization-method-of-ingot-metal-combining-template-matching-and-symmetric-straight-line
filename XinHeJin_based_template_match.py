...
config.enable_device_from_file('C:\\Users\\wujie\\Videos\\20200618_Xin_He_Jin\\realsense\\20200618_135235.bag')
pipeline.start(config)

template_image = cv2.imread("template_2.png", 0)
...
def Box_inside_detect(image, top_left_x, bottom_right_x):   #输入图像必须是单通道的灰度图
    ...
    new_Pt_1 = Line_Difference_v2(mask, step=2, interal=5, point_number=10, state=1)
    new_Pt_2 = Line_Difference_v2(mask, step=1, interal=5, point_number=10, state=0)
...
try:
    while True:
        ...
        depth_colormap = cv2.resize(depth_colormap, None, fx=factor, fy=factor, interpolation=0)
        ...
        # 模板匹配
        res = cv2.matchTemplate(gray_image, template_image, method)
        ...
        #生成多个矩形框
        all_box = Generate_multi_box(gray_image, top_left, bottom_right, state=1, INTERVAL= 115)
        ...
        for i in range(len(all_box)):
            ...
            rect_corner = Box_inside_detect(image=gray_image, top_left_x=temp_top_left, bottom_right_x=temp_bottom_right)
            if rect_corner is not None:
                ...
                #cv2.putText(color_image, str(i), temp_top_left, cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)

        t2 = cv2.getTickCount()
        #画出模板匹配到的矩形框
        #cv2.rectangle(color_image, top_left, bottom_right, (0,0,0), 2)
        #计算运行耗时
        process_time = (t2 - t1) / cv2.getTickFrequency()
        print(process_time)
        fps = float(1 / process_time)
        #cv2.putText(color_image, "FPS: " + str(fps), (0, 20), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)
